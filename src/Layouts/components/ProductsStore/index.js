...
function ProductsStore() {
    ...
    let q = searchParams.get('q'); // Lấy q trên url xuống (Bên ô search)
    ...
    const getProducts = () => {
        fetch(
            `https://vietplayplus.com/api/products?page=${
                pageProps.currentPage
            }&limit=12&sortBy=createdAt:asc&q=${encodeURIComponent(q)}`,
        )
            .then((res) => res.json())
            .then((res) => {
                setPageProps({
        ...
        <div className={cx('wrapper')}>
            <div className={cx('inner')}>
                ...
                {products?.length > 0 ? (
                    ...
                    <div className={cx('not-found')}> Không tìm thấy kết quả bạn mong muốn </div>
                )}
